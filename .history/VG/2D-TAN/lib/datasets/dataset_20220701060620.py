...
class DatasetBase(data.Dataset):
    ...
    def get_iou_map(self, gt_times, duration, num_clips, num_anchors):
        ...
        s_times = torch.arange(0, num_clips).float() * duration / num_clips # [64]
        s_times = s_times[:, None].expand(-1, num_anchors)  # [64, 16]
        lengths = torch.arange(1, num_anchors + 1).float() * duration / num_clips
        lengths = lengths[None, :].expand(num_clips, -1) # [64, 16]
        e_times = s_times + lengths
        overlaps = iou(torch.stack([s_times, e_times], dim=2).view(-1, 2).tolist(), # [64,16,2]->[1024,2]
                       torch.tensor([gt_s_time, gt_e_time]).tolist()).reshape(num_clips, num_anchors) # [2]
        # oberlaps.shape: [64,16]
    ...
    def get_ioa_map(self, gt_times, duration, num_clips, num_anchors):
        gt_s_time, gt_e_time = gt_times # [-8.514829931972784, -1.9502040816326485]
        # duration=40.96, num_clips=64, num_anchors=16
        s_times = torch.arange(0, num_clips).float() * duration / num_clips # torch,size([64])
        s_times = s_times[:, None].expand(-1, num_anchors) # [64, 16]
        lengths = torch.arange(1, num_anchors + 1).float() * duration / num_clips
        # [0.64,1.28,1.92,2.56,...9.6, 10.24]--torch.size([16])
        lengths = lengths[None, :].expand(num_clips, -1)  # [64,16]
        e_times = s_times + lengths  # 
    ...
    def get_video_features(self, video_id):
        ...
        mask = torch.ones(features.shape[0], 1)
         # features.shape=torch.Size([788, 1024])
         # mask.shape=torch.Size([788, 1])
...
class MomentLocalizationDataset(DatasetBase):
    def __init__(self, cfg, split):
        ...
        vocab.stoi['<unk>'] = vocab.vectors.shape[0] # [400000,300]
        vocab.vectors = torch.cat([vocab.vectors, torch.zeros(1, vocab.dim)], dim=0) # [400001, 300]
        word_embedding = nn.Embedding.from_pretrained(vocab.vectors)
        self.vocab = vocab
        self.word_embedding = word_embedding # Embedding(400001, 300)
    ...
    def get_sentence_features(self, description):
        if self.cfg.TXT_INPUT_TYPE == 'glove':
            word_idxs = torch.tensor([self.vocab.stoi.get(w.lower(), 400000) for w in description.split()],
                                     dtype=torch.long)
            # tensor([67, 611, 138, 22479, 220])  [5]
            word_vectors = self.word_embedding(word_idxs) # [5,300]
        else:
            raise NotImplementedError
    ...
    def get_tacos_annotations(self):
        ...
        for vid, video_anno in annotations.items():
            duration = video_anno['num_frames'] / video_anno['fps']   # 29.4
    ...
    def get_sliding_window_item(self, index):
        ...
        # index=1145
        video_id = self.annotations[index]['video']  # video_id='s22-d46'
        gt_s_time, gt_e_time = self.annotations[index]['times']  
        # gt_s_time=52.925170068027214
        # gt_e_time=59.48979591836735
        description = self.annotations[index]['description'] # dexcription='She cut off pineapple top'
        duration = self.annotations[index]['duration'] # duration=504.3537414965987
        time_unit = self.cfg.TIME_UNIT # 0.64
        ...
        num_clips = video_features.shape[0] # 788
        # video_features.shape: torch.Size([384, 4096]), torch.Size([704, 4096]), torch.Size([993, 4096])

        # 这边是对齐token
        if "train" in self.split:
            # 超过64, 就随机取64个clip；否则，全取
            rand_s_idx = random.randrange(
                num_clips - self.cfg.INPUT_NUM_CLIPS) if num_clips > self.cfg.INPUT_NUM_CLIPS else 0
            # 788-64=724内一个随机数作为rand_s_idx
            rand_e_idx = min(rand_s_idx + self.cfg.INPUT_NUM_CLIPS, num_clips)  # 因为要选64个clip
            video_features = video_features[rand_s_idx:rand_e_idx] # [64,1024]从输入视频的788个特征从随机位置开始抽64个
            vis_mask = vis_mask[rand_s_idx:rand_e_idx] # [64,1]
            ...
            if self.cfg.INPUT_NUM_CLIPS > num_clips:
                ...
                gt_s_time = gt_s_time - rand_s_idx * time_unit # -8.514829931972784
                gt_e_time = gt_e_time - rand_s_idx * time_unit # -1.9502040816326485
                if isinstance(self.cfg.OUTPUT_NUM_CLIPS, int) and isinstance(self.cfg.NUM_ANCHORS, int):
                    downsample_rate = self.cfg.INPUT_NUM_CLIPS // self.cfg.OUTPUT_NUM_CLIPS
                    scale_num_clips = max(num_clips // downsample_rate, 1)
                    scale_duration = time_unit * downsample_rate * scale_num_clips
                    ...
                    multi_overlaps, multi_pos_emb = [], []
                    for num_out_clips, num_anchors in zip(self.cfg.OUTPUT_NUM_CLIPS, self.cfg.NUM_ANCHORS):
                        # num_out_clips[64,32,16], num_anchors[16,16,16]
                        downsample_rate = self.cfg.INPUT_NUM_CLIPS // num_out_clips # 1,2,4
                        scale_duration = time_unit * downsample_rate * num_out_clips # 0.64*1*64=40.96
    ...
    def get_sampling_item(self, index):
        ...
        word_vectors, txt_mask = self.get_sentence_features(description)
        video_features, vis_mask = self.get_video_features(video_id)
        video_features = feature_temporal_sampling(self.cfg.INPUT_NUM_CLIPS, video_features) # 64， 
...
class VideoClipDataset(MomentLocalizationDataset):
    def __init__(self, cfg, split):
        super(VideoClipDataset, self).__init__(cfg, split)
        self.annotations = sorted(self.annotations, key=lambda anno: anno['duration'], reverse=True)
        vocab = torchtext.vocab.pretrained_aliases["glove.6B.300d"](
            cache=os.path.join(self.cfg.DATA_DIR, '.vector_cache'))
        vocab.itos.extend(['<unk>'])
    ...
    # def get_sentence_feature(self, description):
    #     return 
    def get_video_features(self, video_id):
        return  