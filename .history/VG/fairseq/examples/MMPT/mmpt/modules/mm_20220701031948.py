...
class MMBertEmbeddings(BertEmbeddings):
    ...
    def forward(
        self,
        input_ids,  # (256,2)
        input_video_embeds,  # [256,32,768]
        token_type_ids=None,  # torch.Size([256, 34])
        position_ids=None,
        inputs_embeds=None,
    ):
      )
        ...
        if position_ids is None:
            ...
            """
            if input_video_embeds is not None:
                video_len = input_video_embeds.size(1)  # 32 
                starting_offset = self.sliding_window + 1 #self.max_video_len + 1  # video [SEP]  # 1025
                ending_offset = self.sliding_window + input_ids.size(1)  # self.max_video_len +   # 1026
            else:
                video_len = 0
                starting_offset = self.max_video_len + 2  # first text token.
                ending_offset = self.max_video_len + input_ids.size(1) + 1
            position_ids = torch.cat([
                self.position_ids[:, :video_len + 1],   # [1,512]->[]
