...
class RetrievalPredictor(Predictor):
    ...
    def predict_loop(
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                index={"idx":data[5], "text_description":data[6]}
                text_description = index["text_description"]  # 256 original text descriptions
                data = self.to_ctx(data)
        ...
        return self.finalize(full_scores, texts, text_description, output_file)

    def __call__(self, sample, full_scores):
        scores = self._get_pooled_outputs(sample)
        self._append_scores(scores, full_scores)

    def finalize(self, full_scores, texts, text_description, output_file=None):
        outputs = self._aggregate_scores(full_scores, text_description)
        if output_file is not None:
    ...
    def _aggregate_scores(self, scores, text_description):
        assert len(scores) == 2
        
        import pdb
        pdb.set_trace()
        
        video_hidden = np.concatenate(scores[0], axis=0)  # [9790, 1402, 768],最长视频是1402s
        text_hidden = np.concatenate(scores[1], axis=0)   # [9790,768]
        
        import h5py
        t=h5py.File('./text_feat.hdf5', 'w')
        tset=t.create_dataset('coords', data=text_hidden)    # [9790,768]
        t.close()
        v=h5py.File('./video_feat.hdf5', 'w')
        vset=v.create_dataset('coords', data=video_hidden)   # video_hidden.shape: [9790,1024,768]
        v.close()
        # clear up.
        self.full_scores = []
