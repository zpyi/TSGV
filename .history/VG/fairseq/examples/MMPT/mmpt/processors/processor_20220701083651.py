...
class TextProcessor(Processor):
    ...
    def __call__(self, text_id):
        ...
        return caption["input_ids"], text_id
...
class Aligner(object):
    ...
    def _build_text_seq(self, text_feature, text_clip_indexs=None):
        ...
        if self.use_window:
            max_text_len = self.max_len - self.sliding_window - 3 # self.max_video_len - 3
            full_caps = full_caps[:max_text_len]
            full_caps = (
            [self.cls_token_id, self.sep_token_id] + full_caps + [self.sep_token_id]
            )
            text_pad_len = self.max_len - len(full_caps) - self.sliding_window # self.max_video_len
        else:
