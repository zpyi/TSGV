...
class DSAligner(Aligner):
    ...
    def __call__(self, video_id, video_feature, text_feature, wps=0.7):
        ...
        vfeats, vmasks = self._build_video_seq(
            video_feature, video_clips  # [424,512], {'start': [0], 'end': [424]}
        )
        # [64], [64]
        caps, cmasks = self._build_text_seq(
            text_feature, text_clip_indexs
        )
        # text_feature['cap']=[[2002, 2165, 2041, 1037, 6090]]
        # text_feature['start']=[0]
        # text_feature['end']=[7.142857142857143]
...
# -------------------- Tacos -----------------------
class TacosMetaProcessor(MetaProcessor):
    def __init__(self,config):
        super().__init__(config)
        ...
        data = []
        for vid, video_anno in json_data.items():
            duration = video_anno['num_frames']
            for timestamp, sentence in zip(video_anno['timestamps'], video_anno['sentences']):
                if timestamp[0] < timestamp[1]:
                    anno={
                            'video': os.path.splitext(vid)[0],
                            'duration': duration,
                            'times': [max(timestamp[0], 0),
                                      min(timestamp[1], duration)],
                            'description': sentence,
                        }
                    data.append(anno)
                    # data.append((anno['video'], anno['description']))
        data = sorted(data, key=lambda anno: anno['duration'], reverse=True)
        self.data = data
        ...
        # self.data = data
    def __getitem__(self, idx):
        return self.data[idx]['video'],self.data[idx]['description']
